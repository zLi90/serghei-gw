...
public:
	...
	// real dt = dt_cfl; // 初始化时间步长为 CFL 时间步长
		...
		// Check for convergence
			...
			// 如果收敛，更新当前时间步长并退出循环
				rt.dt = dt_tmp;
				std::cout << "-------debugzzb-rt.dt000----- " << rt.dt << std::endl;
...
//寻找最大残差值
	...
	// inline void advection_dispersion(RTState &rt, GwState &gw, GwDomain &gdom)
	{
		...
		Kokkos::parallel_for(
			...
			//     rt.advection(iGlob,0) = -0.001 * (rt.c(iGlob, 1) - rt.c(iGlob - 1, 1)) / gdom.dx - 0.002 * (rt.c(iGlob, 1) - rt.c(iGlob - gdom.nxhc, 1)) / gdom.dy - 0.003 * (rt.c(iGlob, 1) - rt.c(iGlob - gdom.nxhc * gdom.nyhc, 1)) / gdom.dz(iGlob);
				...
				//暂时将D值设置为0.1
			    rt.dispersion_x(iGlob,0) = gw.wc(iGlob,0)*0.01 * (rt.c(iGlob + 1, 1) - 2 * rt.c(iGlob, 1) + rt.c(iGlob - 1, 1)) / (gdom.dx * gdom.dx);
			    rt.dispersion_y(iGlob,0) =  gw.wc(iGlob,0)*0.01 * (rt.c(iGlob + gdom.nxhc, 1) - 2 * rt.c(iGlob, 1) + rt.c(iGlob - gdom.nxhc, 1)) / (gdom.dy * gdom.dy);
			    rt.dispersion_z(iGlob,0) = gw.wc(iGlob,0)*0.01 * (rt.c(iGlob + gdom.nxhc * gdom.nyhc, 1) - 2 * rt.c(iGlob, 1) + rt.c(iGlob - gdom.nxhc * gdom.nyhc, 1)) / (gdom.dz(iGlob) * gdom.dz(iGlob));
				
			    rt.dispersion(iGlob,0) = rt.dispersion_x(iGlob,0) + rt.dispersion_y(iGlob,0) + rt.dispersion_z(iGlob,0);
				// std::cout << "-------debugzzb-rt.A+D----- " << rt.advection(iGlob,0) + rt.dispersion(iGlob,0) << std::endl;
			    // Update concentration1
			    rt.c(iGlob, 1) = rt.c(iGlob, 1) + rt.dt * (rt.advection(iGlob,0) + rt.dispersion(iGlob,0));
			//     std::cout << "-------debugzzb-rt.dt----- " << rt.dt << std::endl;
			//更新Cn值
	...
	// 		    real change = fabs(rt.c(iGlob, 1) - rt.c(iGlob, 0));
