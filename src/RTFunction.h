...
public:
	...
	/* --------------------------------------------------
	   RTlinear_system: 组装线性系统 (质量守恒修正版)
	-------------------------------------------------- */
	inline void RTlinear_system(GwState &gw, GwDomain &gdom, RTMatrix &rtA, Parallel &par, RTState &rt, std::vector<RTBC> &rtgbc)
	{
		...
		Kokkos::parallel_for(gdom.nCell, KOKKOS_LAMBDA(int idom) {
			...
			real mass_rhs      = gw.wc_old(iGlob) * rt.c(iGlob, 1); // theta_old * C_old
			...
			if (rt.ReactionModule == 1)
			{
				if (rt.AdsorptionDesorptionModel == Equilibrium_Linear_Model)
				{
					...
					mass_rhs      += rt.rho_b * rt.Kd * rt.c(iGlob, 1);
				}
				else if (rt.AdsorptionDesorptionModel == Equilibrium_Freundlich_Model)
				{
					...
					// RHS 包含 S_old 的贡献
					mass_rhs += rt.rho_b * (rt.Kf * pow(C_curr, rt.Nf)) - rt.rho_b * capacity * C_curr + rt.rho_b * capacity * rt.c(iGlob, 1);
...
//!ck+1,j+1
    ...
    //         iGlob = (hc+kk)*gdom.nxhc*gdom.nyhc + (hc+jj)*gdom.nxhc + ii + hc;
    //         real dc = fabs(rt.c(iGlob,1) - rt.c(iGlob,0)); // 相对变化或绝对变化
	// 		if(dc > tmp) tmp = dc; 
	// 	}, Kokkos::Max<real>(dc_max));
		
	// 	// 简单的自适应策略
	// 	if (dc_max > 0.02) rt.dt *= 0.8;
	// 	else if (dc_max < 0.005) rt.dt *= 1.1;

	// 	if (rt.dt > gdom.dt_max) rt.dt = gdom.dt_max;
	// 	if (rt.dt < gdom.dt_init) rt.dt = gdom.dt_init;
	// }
};
#endif