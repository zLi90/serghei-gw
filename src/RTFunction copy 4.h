...
public:
	...
	// real dt = dt_cfl; // 初始化时间步长为 CFL 时间步长
		...
		// while (tolerance < 0.01) {
			...
			rt.dt = dt_tmp;
			advection_dispersion(rt, gw, gdom);
	...
	// inline void advection_dispersion(RTState &rt, GwState &gw, GwDomain &gdom)
	{
		...
		Kokkos::parallel_for(
		    gdom.nCellMem, KOKKOS_LAMBDA(int iGlob) {
				...
				rt.d(iGlob,0) = (0.3*(pow(rt.aveV(iGlob, 1), 2)+pow(rt.aveV(iGlob, 2), 2))+6*pow(rt.aveV(iGlob, 0), 2))/rt.aveV(iGlob,3);
				rt.d(iGlob,1) = (0.3*(pow(rt.aveV(iGlob, 0), 2)+pow(rt.aveV(iGlob, 2), 2))+6*pow(rt.aveV(iGlob, 1), 2))/rt.aveV(iGlob,3);
				rt.d(iGlob,2) = (0.3*(pow(rt.aveV(iGlob, 0), 2)+pow(rt.aveV(iGlob, 1), 2))+6*pow(rt.aveV(iGlob, 2), 2))/rt.aveV(iGlob,3);
				// std::cout << "-------rt.d(iGlob,0)----- " <<rt.d(iGlob,0) << std::endl;
				//将rt.d中nan值替换
				Kokkos::parallel_for(gdom.nCellMem, KOKKOS_LAMBDA(int iGlob) {
					for (int i = 0; i < 3; ++i) {
						if (std::isnan(rt.d(iGlob,i))) {
							rt.d(iGlob,i) = 1e-6;
						}
					}
					});
				...
				rt.d(iGlob,1) += 1e-6;
				rt.d(iGlob,2) += 1e-6;
				// std::cout << "-------rt.d(iGlob,1)----- " <<rt.d(iGlob,1) << std::endl;		
				
				if (std::isnan(rt.d(iGlob, 0)) || std::isnan(rt.d(iGlob, 1)) || std::isnan(rt.d(iGlob, 2))) {
				std::cout << "NaN value detected in rt.d at index " << iGlob << std::endl;
				std::cout << "rt.d(" << iGlob << ",0): " << rt.d(iGlob, 0) << std::endl;
				std::cout << "rt.d(" << iGlob << ",1): " << rt.d(iGlob, 1) << std::endl;
				std::cout << "rt.d(" << iGlob << ",2): " << rt.d(iGlob, 2) << std::endl;
				}

					
				
				//暂时将D值设置为0.1
			//     rt.dispersion(iGlob,0) = rt.d(iGlob,0) * (rt.c(iGlob + 1, 1) - 2 * rt.c(iGlob, 1) + rt.c(iGlob - 1, 1)) / (gdom.dx * gdom.dx);
			//     rt.dispersion(iGlob,1) =  rt.d(iGlob,1) * (rt.c(iGlob + gdom.nxhc, 1) - 2 * rt.c(iGlob, 1) + rt.c(iGlob - gdom.nxhc, 1)) / (gdom.dy * gdom.dy);
			//     rt.dispersion(iGlob,2) = rt.d(iGlob,2) * (rt.c(iGlob + gdom.nxhc * gdom.nyhc, 1) - 2 * rt.c(iGlob, 1) + rt.c(iGlob - gdom.nxhc * gdom.nyhc, 1)) / (gdom.dz(iGlob) * gdom.dz(iGlob));
			    
			    rt.dispersion(iGlob,0) = gw.wc(iGlob,0)*0.001 * (rt.c(iGlob + 1, 1) - 2 * rt.c(iGlob, 1) + rt.c(iGlob - 1, 1)) / (gdom.dx * gdom.dx);
			    rt.dispersion(iGlob,1) =  gw.wc(iGlob,0)*0.001 * (rt.c(iGlob + gdom.nxhc, 1) - 2 * rt.c(iGlob, 1) + rt.c(iGlob - gdom.nxhc, 1)) / (gdom.dy * gdom.dy);
			    rt.dispersion(iGlob,2) = gw.wc(iGlob,0)*0.001 * (rt.c(iGlob + gdom.nxhc * gdom.nyhc, 1) - 2 * rt.c(iGlob, 1) + rt.c(iGlob - gdom.nxhc * gdom.nyhc, 1)) / (gdom.dz(iGlob) * gdom.dz(iGlob));

			    rt.dispersion(iGlob,3) = rt.dispersion(iGlob,0) + rt.dispersion(iGlob,1) + rt.dispersion(iGlob,2);
				// if (std::isnan(rt.dispersion(iGlob, 0)) || std::isnan(rt.dispersion(iGlob, 1)) || std::isnan(rt.dispersion(iGlob, 2))) {
				// std::cout << "NaN value detected in rt.ddispersion at index " << iGlob << std::endl;
				// std::cout << "rt.dispersion(" << iGlob << ",0): " << rt.dispersion(iGlob, 0) << std::endl;
				// std::cout << "rt.dispersion(" << iGlob << ",1): " << rt.dispersion(iGlob, 1) << std::endl;
				// std::cout << "rt.dispersion(" << iGlob << ",2): " << rt.dispersion(iGlob, 2) << std::endl;
			    ...
			    rt.c(iGlob, 1) = rt.c(iGlob, 1) + rt.dt * (rt.advection(iGlob) + rt.dispersion(iGlob,3));
