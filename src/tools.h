...
public:
  ...
  {
    if (ii < 0)
    { // gauge is undefined, therefore values should be zero, so they can be reduced with MPI_SUM
  ...
  inline std::string printCoordinates()
  {
    ...
    for (int ii = 0; ii < N_SPATIAL_DIM; ii++)
      ss << x(ii) << "\t";
    return (ss.str());
  };
};
...
public:
  ...
  inline void resample()
  {
    ...
    for (int ip = 1; ip < Np; ip++)
    {
      ...
      ig++;
      while (ts + ds < s[ip])
      { // while in the same segment from p[ip-1] to p[ip]
        q[ig] = q[ig - 1] + dv * ds;
    ...
    for (int ig = 0; ig < Ng; ig++)
      std::cout << ig << "\t" << g(ig).x(_X) << "," << g(ig).x(_Y) << std::endl;
    for (int ig = 0; ig < Ng; ig++)
      std::cout << ig << "\t" << sg[ig] << std::endl;
#endif
    delete[] q;
  };
...
public:
  ...
  {
    if (!readGauges(dir, par))
      return 0;
    if (!readLines(dir, par))
      return 0;
    return 1;
  };
  ...
  int readGauges(std::string dir, Parallel const &par)
  {
    ...
    if (fInStream.is_open())
    {
      ...
      if (par.masterproc)
        std::cout << GOK << "Observation gauges read. Number of gauges: " << nGauge << std::endl;
    }
    else
    {
      if (par.masterproc)
        std::cout << BDASH << "Observation gauges file not found. Number of gauges set to zero." << std::endl;
    }
    return 1;
  };
  ...
  int readLines(std::string dir, Parallel const &par)
  {
    ...
    if (fInStream.is_open())
    {
      ...
      {
        if (par.masterproc)
          std::cout << GOK << "No observation lines defined (" << nLines << ")" << std::endl;
        return 1;
      }
      ...
      for (int il = 0; il < nLines; il++)
      {
        ...
        for (int ip = 0; ip < lines[il].Np; ip++)
        {
          for (int kk = 0; kk < N_SPATIAL_DIM; kk++)
          {
            ...
            {
              if (par.masterproc)
                std::cerr << RERROR "Not enough coordinates for observation line" << il << ". Expected " << lines[il].Np << " points, with " << N_SPATIAL_DIM << "coordinates." << std::endl;
              return 0;
            }
          }
        }
      ...
      if (par.masterproc)
        std::cout << GOK << "Observation lines read. Number of lines: " << nLines << std::endl;
    }
    else
    {
      if (par.masterproc)
        std::cout << BDASH << "Observation lines file not found. Number of lines set to zero." << std::endl;
    }
    return 1;
  };
  ...
  {
    if (!configureGauges(dom, outdir))
      return 0;
    if (!configureLines(dom, outdir))
      return 0;
    return 1;
  };
  ...
  int configureGauges(const Domain &dom, const std::string outdir)
  {
    ...
    if (err)
      std::cout << RERROR << "Some gauges were not found" << std::endl;
    ...
    if (dom.id == SERGHEI_MASTERPROC)
      std::cout << GOK << "Observation gauges ready" << std::endl;
    return 1;
  };
  ...
  int configureLines(const Domain &dom, const std::string outdir)
  {
    ...
    for (int il = 0; il < nLines; il++)
    {
      ...
      for (int ig = 0; ig < lines[il].Ng; ig++)
      {
        if (lines[il].g(ig).ic > 0)
          std::cout << GGD << "il=" << il << "\tig=" << ig << "\tindex =" << lines[il].g(ig).ii << "\tcell=" << lines[il].g(ig).ic << "\tdom.id" << dom.id << std::endl;
      }
      MPI_Barrier(MPI_COMM_WORLD);
#endif
      ...
      if (lines[il].mode == OBSLINE_MODE_STATE)
      {
        ...
        for (int iv = 0; iv < nvars; iv++)
        {
          ...
          for (int ig = 0; ig < lines[il].Ng; ig++)
          {
            ObservationGauge &g = lines[il].g(ig);
            if (iv == 0)
              linesOut[jj] << std::scientific << g.sw.h << "\t";
            if (iv == 1)
              linesOut[jj] << std::scientific << g.sw.hu << "\t";
            if (iv == 2)
              linesOut[jj] << std::scientific << g.sw.hv << "\t";
          }
          linesOut[jj] << std::endl;
          jj++;
        }
