	...
	// inline void advection_dispersion(RTState &rt, GwState &gw, GwDomain &gdom)
	{
		...
		Kokkos::parallel_for(
			...
			//     rt.advection(iGlob,0) = -0.001 * (rt.c(iGlob, 1) - rt.c(iGlob - 1, 1)) / gdom.dx - 0.002 * (rt.c(iGlob, 1) - rt.c(iGlob - gdom.nxhc, 1)) / gdom.dy - 0.003 * (rt.c(iGlob, 1) - rt.c(iGlob - gdom.nxhc * gdom.nyhc, 1)) / gdom.dz(iGlob);
				// std::cout << "-------debugzzb-gw.q(iGlob, 0)----- " << rt.advection(iGlob,0) << std::endl;
				// std::cout << "-------debugzzb-gw.q(iGlob, 2)----- " << gw.q(iGlob, 2) << std::endl;
			    // Calculate dispersion contribution

				rt.q_avezzb(iGlob,0) = sqrt(pow(gw.q(iGlob, 0), 2) + pow(gw.q(iGlob, 1), 2) + pow(gw.q(iGlob, 2), 2));
				// std::cout << "-------rt.q_ave(iGlob)----- " <<rt.q_avezzb(iGlob,0) << std::endl;

				
				// if (rt.q_avezzb(iGlob,0)== 0){
				// // 计算 tau_w 的值
    				// rt.tau(iGlob) = pow(gw.wc(iGlob,0), 7.0/3.0) / pow(0.45, 2.0);
				// // std::cout << "-------rt.tau(iGlob)----- " << rt.tau(iGlob) << std::endl;
				// //参考hydrus中弥散度计算
				// rt.d(iGlob,0) = gw.wc(iGlob,0)*0.0001*rt.tau(iGlob);
				// rt.d(iGlob,1) = gw.wc(iGlob,0)*0.0001*rt.tau(iGlob);
				// rt.d(iGlob,2) = gw.wc(iGlob,0)*0.0001*rt.tau(iGlob);
				// std::cout << "-------rt.d(iGlob,0)111----- "  <<rt.d(iGlob,0)<< std::endl;
				// // std::cout << "-------rt.q_ave(iGlob)----- "  << std::endl;
				// }
				// else {
				// rt.d(iGlob,0) = (0.3*(pow(gw.q(iGlob, 1), 2)+pow(gw.q(iGlob, 2), 2))+6*pow(gw.q(iGlob, 0), 2))/rt.q_avezzb(iGlob,0);
				// rt.d(iGlob,1) = (0.3*(pow(gw.q(iGlob, 0), 2)+pow(gw.q(iGlob, 2), 2))+6*pow(gw.q(iGlob, 1), 2))/rt.q_avezzb(iGlob,0);
				// rt.d(iGlob,2) = (0.3*(pow(gw.q(iGlob, 0), 2)+pow(gw.q(iGlob, 1), 2))+6*pow(gw.q(iGlob, 2), 2))/rt.q_avezzb(iGlob,0);
				// std::cout << "-------rt.d(iGlob,0)----- " <<rt.d(iGlob,0) << std::endl;				
				...
				// std::cout << "-------rt.d(iGlob,0)111----- "<<rt.d(iGlob,0) <<std::endl;
				// 循环遍历 rt.d(iGlob,0)
				for (int iGlob = 0; iGlob < rt.c.extent(0); ++iGlob) {
				// 检查是否为 NaN
				if (std::isnan(rt.d(iGlob,0))) {
					// 如果为 NaN，则将其替换为 0.1
					rt.d(iGlob, 0) = 0;
					// std::cout << "-------rt.d(iGlob,0)222----- "<<rt.d(iGlob,0) <<std::endl;	
				}
				}
				for (int iGlob = 0; iGlob < rt.c.extent(0); ++iGlob) {
				// 检查是否为 NaN
				if (std::isnan(rt.d(iGlob,1))) {
					// 如果为 NaN，则将其替换为 0.1
					rt.d(iGlob, 1) = 0;
				}
				}
				...
				// // 计算 tau_w 的值
    				// rt.tau(iGlob) = pow(gw.wc(iGlob,0), 7.0/3.0) / pow(0.45, 2.0);
				// //参考hydrus中弥散度计算
				// rt.d(iGlob,0) = (0.3*(pow(gw.q(iGlob, 1), 2)+pow(gw.q(iGlob, 2), 2))+6*pow(gw.q(iGlob, 0), 2))/rt.q_avezzb(iGlob,0)+gw.wc(iGlob,0)*0.0001*rt.tau(iGlob);
				// rt.d(iGlob,1) = (0.3*(pow(gw.q(iGlob, 0), 2)+pow(gw.q(iGlob, 2), 2))+6*pow(gw.q(iGlob, 1), 2))/rt.q_avezzb(iGlob,0)+gw.wc(iGlob,0)*0.0001*rt.tau(iGlob);
				// rt.d(iGlob,2) = (0.3*(pow(gw.q(iGlob, 0), 2)+pow(gw.q(iGlob, 1), 2))+6*pow(gw.q(iGlob, 2), 2))/rt.q_avezzb(iGlob,0)+gw.wc(iGlob,0)*0.0001*rt.tau(iGlob);
				// std::cout << "-------rt.d(iGlob,0)222----- "  <<rt.d(iGlob,0)<< std::endl;
				// // }

				// std::cout << "-------rt.d(iGlob,0)----- " <<rt.d(iGlob,0) << std::endl;
				//暂时将D值设置为0.1
			    rt.dispersion_x(iGlob,0) = gw.wc(iGlob,0)*1e-06 * (rt.c(iGlob + 1, 1) - 2 * rt.c(iGlob, 1) + rt.c(iGlob - 1, 1)) / (gdom.dx * gdom.dx);
			    rt.dispersion_y(iGlob,0) = gw.wc(iGlob,0)*  1e-06 * (rt.c(iGlob + gdom.nxhc, 1) - 2 * rt.c(iGlob, 1) + rt.c(iGlob - gdom.nxhc, 1)) / (gdom.dy * gdom.dy);
			    rt.dispersion_z(iGlob,0) = gw.wc(iGlob,0)*1e-06 * (rt.c(iGlob + gdom.nxhc * gdom.nyhc, 1) - 2 * rt.c(iGlob, 1) + rt.c(iGlob - gdom.nxhc * gdom.nyhc, 1)) / (gdom.dz(iGlob) * gdom.dz(iGlob));
			
			    rt.dispersion(iGlob,0) = rt.dispersion_x(iGlob,0) + rt.dispersion_y(iGlob,0) + rt.dispersion_z(iGlob,0);
				// std::cout << "-------rt.dispersion_x(iGlob,0)----- " <<rt.dispersion(iGlob,0) << std::endl;
