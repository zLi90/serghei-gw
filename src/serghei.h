...
// Unify dt
#if SERGHEI_SWE_MODEL
			tint.computeDt(state, dom, io);
#if SERGHEI_SUBSURFACE_MODEL	  //地下水模块开启
			if (!gdom.async) // 同步耦合
			{
				if (dom.dt < gdom.dt)
				{
				...
				// 同步耦合情况下，选择dom.dt和gdom.dt中较小值作为最终时间步长
			}
			// 异步耦合时，地表水dt大于地下水dt，则地表水dt选择较小的地下水dt
			// 地表水dt小于地下水dt时，地表水dt和地下水dt取不同值
			...
			*/
			...
			// if (par.masterproc)
			// {
			// 	std::cerr << "Time step: " << dom.dt << std::endl;
			// }
			// dom.countIterDt = 0;
			// accumDt = 0.0;
			// if (dom.etime >= dom.endTime)
			// {
			// 	break;
			// }
		} // end of time loop
