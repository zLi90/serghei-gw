                      ...
                      Parallel &par, FileIO &io, SourceSink &ss, std::string inFolder, std::string outFolder)
    {
        ...
        for (iGlob = 0; iGlob < gdom.nCellMem; iGlob++)
        {
            ...
            else
            {
                ...
                if (gdom.z(iGlob + 1) > gdom.z(iGlob))
                {
                    gdom.sinx(iGlob) = hdiff / dist;
                }
                else
                {
                    gdom.sinx(iGlob) = -hdiff / dist;
            ...
            else
            {
                ...
                if (gdom.z(iGlob + gdom.nxhc) > gdom.z(iGlob))
                {
                    gdom.siny(iGlob) = hdiff / dist;
                }
                else
                {
                    gdom.siny(iGlob) = -hdiff / dist;
    ...
    int readGwFile(std::string fNameIn, GwDomain &gdom, Parallel &par)
    {
        ...
        if (fInStream.is_open())
        {
            while (std::getline(fInStream, line))
            {
                ...
                if (!pline.key.empty())
                {
                    if (!strcmp("ndepth", pline.key.c_str()))
                    {
                        pline.value >> gdom.nz_glob;
                    }
                    else if (!strcmp("height", pline.key.c_str()))
                    {
                        pline.value >> gdom.thickH;
                    }
                    else if (!strcmp("dz_multiplier", pline.key.c_str()))
                    {
                        pline.value >> gdom.dz_multiplier;
                    }
                    else if (!strcmp("dz_base", pline.key.c_str()))
                    {
                        pline.value >> gdom.dz_base;
                    }
                    else if (!strcmp("dt_init", pline.key.c_str()))
                    {
                        pline.value >> gdom.dt_init;
                    }
                    else if (!strcmp("dt_max", pline.key.c_str()))
                    {
                        pline.value >> gdom.dt_max;
                    }
                    else if (!strcmp("nSoilID", pline.key.c_str()))
                    {
                        pline.value >> gdom.nSoilID;
                    }
                    else if (!strcmp("gw_scheme", pline.key.c_str()))
                    {
                        pline.value >> gdom.gw_scheme;
                    }
                    else if (!strcmp("cg_iter", pline.key.c_str()))
                    {
                        pline.value >> gdom.cg_iter;
                    }
                    else if (!strcmp("cg_tol", pline.key.c_str()))
                    {
                        pline.value >> gdom.cg_tol;
                    }
                    else if (!strcmp("aev", pline.key.c_str()))
                    {
                        pline.value >> gdom.aev;
                    }
                    else if (!strcmp("async", pline.key.c_str()))
                    {
                        pline.value >> gdom.async;
                    }
                }
            }
        }
        ...
        {
            if (par.masterproc)
                std::cerr << RERROR "key " << "ndepth" << " not set.";
            exit(-1);
        }
        if (gdom.dz_multiplier == -999)
        {
            if (par.masterproc)
                std::cerr << RERROR "key " << "dz_multiplier" << " not set.";
            exit(-1);
        }
        if (gdom.thickH == -999)
        {
            if (par.masterproc)
                std::cerr << RERROR "key " << "height" << " not set.";
            exit(-1);
        }
        if (gdom.dt_init == -999)
        {
            if (par.masterproc)
                std::cerr << RERROR "key " << "dt_init" << " not set.";
            exit(-1);
        }
        if (gdom.dt_max == -999)
        {
            if (par.masterproc)
                std::cerr << RERROR "key " << "dt_max" << " not set.";
            exit(-1);
        }
        if (gdom.gw_scheme == -999)
        {
            if (par.masterproc)
                std::cerr << RERROR "key " << "gw_scheme" << " not set.";
            exit(-1);
        }
        if (gdom.aev == -999)
        {
            if (par.masterproc)
                std::cerr << RERROR "key " << "aev" << " not set.";
            exit(-1);
        }
        if (gdom.async == -999)
        {
            if (par.masterproc)
                std::cerr << RERROR "key " << "async" << " not set.";
            exit(-1);
        }
        if (gdom.dz_multiplier > 1 && gdom.dz_base == -999)
        {
            if (par.masterproc)
                std::cerr << RERROR "key " << "dz_base" << " not set.";
            exit(-1);
        }
    ...
    int readVGParameters(std::string fNameIn, GwState &gw, GwDomain &gdom, Parallel &par)
    {
        ...
        else
        {
            ...
            else
            {
                if (par.masterproc)
                {
                    std::cerr << RERROR "Unable to open VG parameters file!\n";
                    return 0;
                }
            }
        }
    ...
    int readSoilID(std::string fNameIn, GwState &gw, GwDomain &gdom, Parallel &par)
    {
        ...
        if (fInStream.is_open())
        {
            ...
            if (n_soil != gdom.nSoilID)
            {
                if (par.masterproc)
                {
                    std::cerr << RERROR "Number of soilID != that specified in the VG Table. Unable to continue\n";
                }
                if (par.masterproc)
                {
                    std::cerr << BDASH "n_soil: " << n_soil << ", in the Table : " << NVG << "\n";
                }
                return 0;
            }
            if (n_soil == 1)
            ...
            else
            {
                for (int ii = 0; ii < ndata; ii++)
                {
                    ...
                    else
                    {
                        if (par.masterproc)
                        {
                            std::cerr << RERROR "Error reading soil ID file. Not enough data\n";
                            return 0;
                        }
                    }
                }
        ...
        else
        {
            ...
            else
            {
                if (par.masterproc)
                {
                    std::cerr << RERROR "Unable to open soil ID file!\n";
                    return 0;
                }
            }
        }
    ...
    int readGwBCFile(std::string fNameIn, GwDomain &gdom, SubsurfaceBoundaries &gbc, Parallel &par, GwState &gw)
    {
        ...
        if (fInStream.is_open())
        {
            while (std::getline(fInStream, line))
            {
                ...
                if (!pline.key.empty())
                {
                    ...
                    else if (!strcmp("id", pline.key.c_str()))
                    {
                        ...
                        if (bccount > 0 && ibc >= 0)
                        {
                            pline.value >> gbc.id[ibc];
                        }
                    }
                    else if (!strcmp("bctype", pline.key.c_str()) && ibc >= 0)
                    {
                        if (bccount > 0)
                            pline.value >> gbc.gwbc[ibc].bctype;
                    }
            ...
            else
            {
                if (par.masterproc)
                {
                    std::cerr << GOK "Subsurface BC set\n";
                }
            }
        }
        else
        {
            if (par.masterproc)
            {
                std::cerr << YEXC << "gwbc.input not found. Default boundaries used." << std::endl;
            }
        }
        // Read polygon file
        for (int k = 0; k < polygonFile.size(); k++)
        {
            ...
            if (fPoly.is_open())
            {
                ...
                if (!gbc.gwbc[k].find_bcells(gw, gbc.id[k], gdom, par, nPoly, xPoly, yPoly))
                    return 0;
            }
            else
            {
        ...
        for (int k = 0; k < tsFile.size(); k++)
        {
            ...
            if (readts)
            {
                if (fts.is_open())
                {
                    ...
                    for (int i = 0; i < ndatat; i++)
                    {
                        if (!fts.fail() && !fts.eof())
                        {
                            ...
                            if (gbc.gwbc[k].ts.nc <= 1)
                            {
                                fts >> gbc.gwbc[k].ts.value(i);
                            }
                            else
                            {
                ...
                else
                {
                    if (par.masterproc)
                    {
                        std::cerr << RERROR "Error opening timeseries file " << fname << std::endl;
                        return 0;
                    }
                }
            }
            ...
            if (hasbcfile && readbc && fbc.good())
            {
                ...
                if (fbc.is_open())
                {
                    ...
                    if (nx * ny * nz != ndata)
                    {
                        ...
                        std::cerr << RERROR "Error reading bc data file " << bcFile[k] << " nx*ny*nz != ndata! " << std::endl;
                        return 0;
                    }
                    if (ndata > 0)
                    {
                ...
                else
                {
                    if (par.masterproc)
                    {
                        std::cerr << RERROR "Error opening bc file " << fname << std::endl;
                        return 0;
                    }
                }
            }
        ...
        if (par.masterproc)
            std::cout << GOK << "Subsurface boundary file parsed and boundaries set" << std::endl;
        return 1;
    }
    ...
    int readGwSSFile(std::string fNameIn, GwState &gw, GwDomain &gdom, SourceSink &ss, Parallel &par)
    {
        ...
        if (fInStream.is_open())
        {
            while (std::getline(fInStream, line))
            {
                ...
                if (!pline.key.empty())
                {
                    ...
                    else if (!strcmp("id", pline.key.c_str()))
                    {
                        ...
                        if (sscount > 0 && iss >= 0)
                        {
                            pline.value >> ss.id[iss];
                        }
                    }
                    else if (!strcmp("sstype", pline.key.c_str()) && iss >= 0)
                    {
                        if (sscount > 0)
                            pline.value >> ss.gwss[iss].sstype;
                    }
            ...
            else
            {
                if (par.masterproc)
                {
                    std::cerr << GOK "Subsurface Source/Sink set\n";
                }
            }
        }
        else
...
// printf("ii: %d, time: %f, ss.swss.evap.time(ii): %f, ss.swss.evap.value(ii): %e\n", ii, ss.swss.evap.time(ii), ss.swss.evap.time(ii), ss.swss.evap.value(ii));
        ...
        if (par.masterproc)
            std::cout << GOK << "Subsurface source/sink file parsed set" << std::endl;
        return 1;
    }
    ...
    int setGwState(std::string inFolder, GwState &gw, GwDomain &gdom, State &state, SubsurfaceBoundaries &gbc, Parallel &par, FileIO &io)
    {
        ...
        if (read)
        {
            ...
            else
            {
                if (read == 1)
                {
                    if (par.masterproc)
                    {
                        std::cerr << RERROR "File " << inFolder + "subsurface.input" << " not found" << std::endl;
                        return 0;
                    }
                }
            }
        ...
        else
        {
            if (par.masterproc)
            {
                std::cerr << RERROR "Initial mode must be IC_SAT(1), IC_H(2), IC_WC(3) or IC_WT(4)!" << std::endl;
                return 0;
            }
        }
        if (par.masterproc)
    ...
    */
    int readGwICFile(std::string fNameIn, std::string fDirIn, GwState &gw, GwDomain &gdom, Parallel &par)
    {
        ...
        int ndata;
        if (!strcmp(fNameIn.c_str(), "wt.input"))
        {
            ndata = gdom.nx_glob * gdom.ny_glob;
        }
        else
        {
            ndata = gdom.nx_glob * gdom.ny_glob * gdom.nz_glob;
        }
        ...
        if (fInStream.is_open())
        {
            ...
            for (int ii = 0; ii < ndata; ii++)
            {
                ...
                else
                {
                    if (par.masterproc)
                    {
                        std::cerr << RERROR "Error reading IC file. Not enough data\n";
                        return 0;
                    }
                }
            }
        ...
        else
        {
            if (par.masterproc)
            {
                std::cerr << RERROR "Error reading head/theta IC file. File name might be wrong.\n";
                return 0;
            }
        }
        if (par.masterproc)
        {
            std::cerr << GOK "Subsurface head/water content set\n";
        }
        return 1;
    }
};
