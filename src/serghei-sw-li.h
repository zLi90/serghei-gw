...
public:
    ...
    init.initializeMPI( &argc , &argv , par );
        ...
        if (!rtinit.initialize_rt(rt, par, inFolder))   {
			std::cerr << RERROR "Unable to initialize the reactive transport module" << "\n";
			return 0;
        }
        // reactive transport for SWE
        #if SERGHEI_SWE_MODEL
        if (!rtinitsw.initialize_rt(rtsw, rt, state, dom, ss.swss, ebc, io, par, inFolder, outFolder)) {
			std::cerr << RERROR "Unable to initialize the SW transport module" << "\n";
			return 0;
        }
        #endif
        // reactive transport for RE
        /*#if SERGHEI_SUBSURFACE_MODEL
		if (!rtinit.initialize_rt(rt, gw, gdom, rtgbc, gmpi, par, io, ss, inFolder, outFolder)) {
			std::cerr << RERROR "Unable to initialize the GW transport module" << "\n";
			return 0;
		}
		rtA.init(gdom);
...
#if SERGHEI_REACTIVE_TRANSPORT
            rtfsw.transport(rtsw, state, dom, ebc);
#endif
			#else
...
//!zzb solve the reactive transport equation
#if SERGHEI_SUBSURFACE_TRANSPORT
...
//!zzb	
        ...
        std::cout << RERROR << "Impossible configuration without SWE nor GW model" << std::endl;
				return 0;
#if SERGHEI_SUBSURFACE_TRANSPORT // 地下水传输模块开启
			// if (rt.dt < dom.dt)
			// {
			// 	dom.dt=rt.dt;
			// 	// rt.dt = dom.dt;
