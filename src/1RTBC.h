...
#define SUB_BC_Dirichlet_CONST 2
#define SUB_BC_Neumann_CONST 3
#define SUB_BC_Cauchy_CONST 4
#define SUB_BC_Dirichlet_T 5
#define SUB_BC_Neumann_T 6
#define SUB_BC_Cauchy_T 7
#define SUB_BC_SWE 8
#define SUB_BC_FD 9
...
public:
	...
	inline int find_bcells(GwState &gw, std::string &rtid, GwDomain &gdom, Parallel &par, int nPoly, realArr &xPoly, realArr &yPoly){
		...
		if(ncells_all>0){
            ...
            if (RTbctype == SUB_BC_SWE)   {swgw_type = intArr("swgw_type", ncellsBC);}
			#ifdef __NVCC__
    ...
    inline void applyDirichletBC(RTState &rt, GwState &gw, GwDomain &gdom, Parallel &par) {
        ...
        if (ncellsBC > 0 && onBoundary == 1) {
            ...
            if (RTbctype == SUB_BC_Dirichlet_T ) {Conbc = interpolateLinear(ts, gdom.etime);}
		...
		// 			gw.h(iGhost,1) = gw.h(iGlob,1);
          ...
          else if (RTbctype == SUB_BC_Dirichlet_CONST || RTbctype == SUB_BC_Neumann_CONST || RTbctype == SUB_BC_Cauchy_CONST){
				Kokkos::parallel_for("rt_bc_c", ncellsBC, KOKKOS_CLASS_LAMBDA (int ibc){
                    int iGlob = bcells[ibc], iGhost = gcells[ibc];
                         rt.c(iGhost,1) = rtbcvals(ibc);

                });
			}
            // H Time series
            else if (RTbctype == SUB_BC_Dirichlet_T)  {
                Kokkos::parallel_for("rt_bc_c", ncellsBC, KOKKOS_CLASS_LAMBDA (int ibc){
                    int iGlob = bcells[ibc], iGhost = gcells[ibc];
                    rt.c(iGhost,1) = Conbc;
                });
    ...
    inline void applyRTMatBC(RTState &rt, GwState &gw, GwDomain &gdom, Parallel &par) {
        ...
        if (ncellsBC > 0 && onBoundary == 1) {
            real Neumannbc;
            if (RTbctype == SUB_BC_Neumann_T) {Neumannbc = interpolateLinear(ts, gdom.etime);}
	  	    switch (RTbctype) {
                case SUB_BC_Dirichlet_CONST:    
               //  case SUB_BC_Cauchy_CONST:   
                case SUB_BC_Dirichlet_T:    
               //  case SUB_BC_Cauchy_T:
			...
			// 			else if (direction == 6)	{
			// 				rt.RTcoef(idom,7) -= gdom.dt * gw.k(iGlob-gdom.nxhc*gdom.nyhc,2) / gdom.dz(iGlob);
			// 				rt.RTcoef(idom,7) -= gdom.dt * bcvals(ibc) / gdom.dz(iGlob);
