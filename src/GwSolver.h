...
public:
	...
	*/
	...
	void kkpcg(GwMatrix &A)
	{
		...
		KokkosKernels::Experimental::Example::pcgsolve(kh, matA, rhs, vecx, diag, cg_iteration_limit, cg_iteration_tolerance, &cg_result, usePreconditioner);
		Kokkos::fence();
	...
	*/
	...
	{
		Kokkos::parallel_for(A.nrow, KOKKOS_LAMBDA(int idom) { A.z(idom) = A.r(idom) * A.diag(idom); });
	}
	...
	*/
	...
	{
		Kokkos::parallel_for(A.nrow, KOKKOS_LAMBDA(int idx) { A.x(idx) += alpha * A.p(idx); });
	}
